...
public interface GiftCouponRepository extends PagingAndSortingRepository<GiftCoupon, Integer> {
    ...
    @Query("select g from GiftCoupon g where g.valid=:valid and g.beginTime<:today and :today<g.endTime and g.name like CONCAT('%',:name,'%') ")
    Page<GiftCoupon> findAllByValidAndDateBetweenAndNameLike(@Param("valid") ValidEnum VALID, @Param("name") String name, @Param("today") Calendar calendar,
                                                             Pageable pageable);
