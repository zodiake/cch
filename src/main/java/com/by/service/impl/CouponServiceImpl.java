...
public class CouponServiceImpl implements CouponService {
    ...
    public Page<CouponJson> findAll(Pageable pageable) {
        Page<GiftCoupon> giftCoupons = giftCouponService.findAllByValidAndDateBetween(ValidEnum.VALID,
                Calendar.getInstance(), pageable);
        Page<ShopCoupon> shopCoupons = shopCouponService.findAllByValidAndDateBetween(ValidEnum.VALID,
                Calendar.getInstance(), pageable);
    ...
    public Page<CouponJson> findByMember(Member member, Pageable pageable) {
        Page<GiftCouponMember> giftCouponList = giftCouponMemberRepository.findByMemberAndValid(member, ValidEnum.VALID,
                Calendar.getInstance(), pageable);
        Page<ShopCouponMember> shopCouponMemberList = shopCouponMemberRepository.findByMemberAndValid(member,
                ValidEnum.VALID, Calendar.getInstance(), pageable);
        List<CouponJson> results = new ArrayList<>();
        List<CouponJson> preferentialJson = giftCouponList.getContent().stream().map(i -> {
            CouponJson json = new CouponJson(i.getCoupon());
            json.setType("g");
            json.setCode(i.getCode());
            return json;
        }).collect(Collectors.toList());
        List<CouponJson> shopJson = shopCouponMemberList.getContent().stream().map(i -> {
            CouponJson json = new CouponJson(i.getCoupon());
            json.setType("s");
            json.setCode(i.getCode());
            return json;
    ...
    public Page<CouponJson> findByNameLike(String name, Pageable pageable) {
        Page<GiftCoupon> giftCoupons = giftCouponService.findAllByValidAndDateBetweenAndNameLike(ValidEnum.VALID, name, Calendar.getInstance(), pageable);
        Page<ShopCoupon> shopCoupons = shopCouponService.findAllByValidAndDateBetweenAndNameLike(ValidEnum.VALID, Calendar.getInstance(), name, pageable);
        List<CouponJson> results = new ArrayList<>();
        List<CouponJson> giftJson = giftCoupons.getContent().stream().map(i -> {
            CouponJson json = new CouponJson(i);
            json.setType("g");
